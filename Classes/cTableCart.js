//----------------------------------------------------------------------------------------------------
//	Classes/cTableCart.js
//
//	generate signal "change" on selection changes.
//----------------------------------------------------------------------------------------------------

//alert("Classes/cTableCart.js");

//----------------------------------------------------------------------------------------------------
//	cTableCart namespace.
//----------------------------------------------------------------------------------------------------
var cTableCart = {};
...
{
	if (!fIsString(vClassName)) fErr("cTableCart.fAdd() invalid vClassName: " + vClassName);
	if (!fIsNumber(vIndex)) fErr("cTableCart.fAdd() invalid vIndex: " + vIndex);
	if (!fIsObject(vData)) fErr("cTableCart.fAdd() invalid vData: " + vData);
	...
	for (i = 0; i < vThis.tFoot.rows.length; i++)
		if (vThis.tFoot.rows[i].className == vClassName)
			break;
	if (i == vThis.tFoot.rows.length) fErr("cTableCart.fAdd() invalid vClassName: " + vClassName);
	...
	for (i = 0; i < vTr.cells.length; i++)
		for (vKey in vData)
			vTr.cells[i].innerHTML = vTr.cells[i].innerHTML.replace(new RegExp("~" + vKey + "\\b", 
				"g"), vData[vKey]);
