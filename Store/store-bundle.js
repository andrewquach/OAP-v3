...
)
{
  ...
  switch (vSignal)
  {
      ...
      cPage.mBookPanel = new cBookPanel;
      fInitBookPanel();
     return;
      ...
      var vBundleItems = vData.Data.bundleItems;
      ...
      for (var i=0; i<1; i++)
      {
        ...
        elm = fGet('#ePrice');
      	...
      	vAuthor = (vData[i].Author == undefined)?"":vData[i].Author;
		...
		for (var i=0; i<vBundleItems.length; i++)
      	{
      		if (vBundleItems[i].Level == "34")
      			vBundleItems[i].Level = 'Primary 3/4';
      		else if (vBundleItems[i].Level == "56")
      			vBundleItems[i].Level = 'Primary 5/6';
      		else if (vBundleItems[i].Level == "12")
      			vBundleItems[i].Level = 'Primary 1/2';
      		else
        		vBundleItems[i].Level = 'Primary ' + vBundleItems[i].LevelId;
        	vBundleItems[i].Cover = "res/" + vBundleItems[i].Cover;
        	cPage.mBookPanel.fSetActive(vBundleItems[i].Id);
        	cPage.mBookPanel.fAdd(vBundleItems[i]);
        	
        	vFullTitle = vBundleItems[i].Title+" - "+vBundleItems[i].Level;
      		vItems.push({'data': vBundleItems[i].Id, 'text': vFullTitle});

      	}
      	
      	fResetDropDownItems({'div': 'eBundleItemDiv', 'id': 'eBundleItems'}, vItems);
      	fRefresh(cPage.mThis);	 	
      }
  ...
  cPage.mBookPanel.fSignal({'signal': vSignal, 'target': vTarget});
...
//----------------------------------------------------------------------------------------------------
function fInitBookPanel(
)
{
  cPage.mBookPanel.fInit({
    'ctrls': {
      'sampleIt': {'id': 'eSampleIt'},
      'previewCW': {'id': 'ePreviewCW'}
    },
    'fields' : {'Title': 'eTitleItem', 'Publisher': 'ePublisherItem', 'Author': 'eAuthorItem', 'AuthorLi': 'liAuthor', 
      'LevelId': 'eLevelItem', 'Cover': 'eCoverItem', 'Synopsis': 'eSynopsisItem'},
    'notify': function(vTarget) {
      //fRefresh(vTarget);
    }
  });
}
...
)
{
  ...
  fGet('#' + vTarget.div).innerHTML = 
    '<select id=\'' + vTarget.id + '\' onchange="fOnDropListChg(this)">' + vOpts + '</select>';  
}

//----------------------------------------------------------------------------------------------------
//  invoke actions when drop-lists changed.
//----------------------------------------------------------------------------------------------------
function fOnDropListChg(
  vElm
)
{
  var vId = vElm.getAttribute('id');
  if (vId == 'eBundleItems')
    fRefresh(cPage.mThis);
}

//----------------------------------------------------------------------------------------------------
//  refresh page based on current states.
//----------------------------------------------------------------------------------------------------
function fRefresh(
  vThis
)
{
  cPage.mBookPanel.fSetActive(fCurrItem());
  cPage.mBookPanel.fRefresh();
}
...
{
  return fDropListData('eBundleItems');
}
