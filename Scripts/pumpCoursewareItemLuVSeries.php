...
for ($i=0; !feof($vFp); $i++)
{
	...
	$vQuery = "UPDATE tblCoursewareItem
			   SET Title = '" . mysql_real_escape_string($vRow["Title"]) . "',
				   ...
				   res = 'res/$vSeries/$folder/". mysql_real_escape_string($vRow["res"]) ."'
				WHERE Id = ".$vRow["Id"];
	//echo $vQuery;
	mysqli_query($vConn, $vQuery);
	fCheckLastQuery($vConn, $vQuery);
	
	if (!file_exists("../res/$vSeries/$folder/". mysql_real_escape_string($vRow["res"])))
		echo "Please check the path of 'res/$vSeries/$folder/". mysql_real_escape_string($vRow["res"]) ."<br/>";
}
